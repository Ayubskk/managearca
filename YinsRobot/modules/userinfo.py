...
from YinsRobot.__main__ import STATS, TOKEN, USER_INFO
from YinsRobot.modules.sql import SESSION
import YinsRobot.modules.sql.userinfo_sql as sql
from YinsRobot.modules.disable import DisableAbleCommandHandler
from YinsRobot.modules.sql.global_bans_sql import is_user_gbanned
from YinsRobot.modules.sql.afk_sql import is_afk, set_afk
from YinsRobot.modules.sql.users_sql import get_user_num_chats
from YinsRobot.modules.helper_funcs.chat_status import sudo_plus
from YinsRobot.modules.helper_funcs.extraction import extract_user
...
def info(update: Update, context: CallbackContext):
    ...
    if user_id not in [bot.id, 777000, 1376065420]:
                    ...
                    [
                        [
                            InlineKeyboardButton(
                                "Health", url="https://t.me/"),
                            InlineKeyboardButton(
                                "Disaster", url="https://t.me/")
                        ],
                    ]
                ),
                parse_mode=ParseMode.HTML,
...
def set_about_me(update: Update, context: CallbackContext):
    ...
    if user_id in [777000, 1376065420]:
    ...
    if message.reply_to_message:
        ...
        if repl_user_id in [bot.id, 777000, 1376065420] and (user_id in DEV_USERS):
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id in [777000, 1376065420] and sender_id not in DEV_USERS:
